...
sudo ovs-ofctl add-flow s1 ip,nw_src=10.0.1.1/32,actions=set_queue:4,output:6
sudo ovs-ofctl add-flow s1 in_port=6,actions=normal
sudo ovs-ofctl add-flow s2 ip,nw_src=10.0.4.0/32,actions=set_queue:5,output:5
sudo ovs-ofctl add-flow s2 ip,nw_src=10.0.3.0/32,actions=set_queue:6,output:5
sudo ovs-ofctl add-flow s2 in_port=5,actions=normal
sudo ovs-ofctl add-flow s2 ip,nw_src=10.0.3.1/32,actions=set_queue:7,output:6
sudo ovs-ofctl add-flow s2 in_port=6,actions=normal
sudo ovs-ofctl add-flow s3 ip,nw_src=10.0.6.0/32,actions=set_queue:8,output:3
sudo ovs-ofctl add-flow s3 ip,nw_src=10.0.5.0/32,actions=set_queue:9,output:3
sudo ovs-ofctl add-flow s3 in_port=3,actions=normal
sudo ovs-ofctl add-flow s4 ip,nw_src=10.0.8.0/32,actions=set_queue:10,output:5
sudo ovs-ofctl add-flow s4 ip,nw_src=10.0.7.0/32,actions=set_queue:11,output:5
sudo ovs-ofctl add-flow s4 in_port=5,actions=normal
sudo ovs-ofctl add-flow s4 ip,nw_src=10.0.7.1/32,actions=set_queue:12,output:6
sudo ovs-ofctl add-flow s4 in_port=6,actions=normal
sudo ovs-ofctl add-flow s5 ip,nw_src=10.0.9.0/32,actions=set_queue:13,output:5
sudo ovs-ofctl add-flow s5 in_port=5,actions=normal
sudo ovs-ofctl add-flow s5 ip,nw_src=10.0.10.1/32,actions=set_queue:14,output:6
sudo ovs-ofctl add-flow s5 ip,nw_src=10.0.9.1/32,actions=set_queue:15,output:6
