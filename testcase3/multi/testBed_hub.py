...
def WifiNet(inputFile):
    ...
    # start D-ITG Servers
        ...
        for i in range(0, num_host - 1):
            sender = i
            receiver = num_host - 1
            #iperfTest(sender,receiver,hosts,nodes)
        ...
        for i in range(0, num_host - 1):
            ...
            for j in range(host_eth[i]-1,-1,-1):
                ...
                out_f="host"+str(i)+"eth"+str(host_eth[i]-1-j)+"_mptcp.stat"
...
def iperfTest(srcNo, dstNo, hosts,nodes):
    ...
    src.cmdPrint("iperf -c 10.0.0." + str(dstNo + 1) + " -t 60 -i 2 &")
    time.sleep(0.2)
